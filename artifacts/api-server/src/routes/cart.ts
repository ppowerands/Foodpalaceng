...
import { cartItemsTable, productsTable, productVariantsTable, addonsTable } from "@workspace/db";
...
async function buildCart(userId: number) {
  ...
  const enriched = await Promise.all(items.map(async (item) => {
    ...
    if (addonIds.length > 0) {
      ...
      addonTotal = addons.filter(Boolean).reduce((sum, a) => sum + parseFloat(a!.price as unknown as string), 0);
    }
...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { productId, variantId, addonIds, quantity, specialInstructions } = req.body;
    const [product] = await db.select().from(productsTable).where(eq(productsTable.id, productId)).limit(1);
    if (!product) return res.status(404).json({ error: "Product not found" });
    if (!product.inStock || !product.isAvailable) return res.status(400).json({ error: "Product not available" });
    ...
    if (variantId) {
      ...
      if (variant) unitPrice = parseFloat(variant.price as unknown as string);
    }
    ...
    if (existing.length > 0 && !variantId && (!addonIds || addonIds.length === 0)) {
      await db.update(cartItemsTable).set({ quantity: existing[0]!.quantity + (quantity ?? 1) }).where(eq(cartItemsTable.id, existing[0]!.id));
    } else {
      await db.insert(cartItemsTable).values({
        userId: req.user!.id, productId, variantId: variantId ?? null,
        addonIds: addonIds ?? [], quantity: quantity ?? 1,
        unitPrice: String(unitPrice), specialInstructions: specialInstructions ?? null,
      });
    }
